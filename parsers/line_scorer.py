...
_BIOMARKER_RUS_FRAGMENTS = [
    ...
    "железо", "витамин",
    # Дополнительные биомаркеры (Гемотест, биохимия)
    "гликированный гемоглоб",
    "мочевая кислота",
    "с-реактивный белок",
    "триглицерид",
    "липопротеин",
    ...
    "сывороточное железо",
    "общий белок",
    # Новые фрагменты P5-A
    "тиреотропн", "тироксин", "трийодтиронин",
    "протромбин", "фибриноген", "д-димер", "d-димер",
    "ферритин", "трансферрин", "железосвязыв",
    "ревматоидный фактор", "антистрептолизин",
    "гомоцистеин", "инсулин", "с-пептид",
    "цистатин", "фруктозамин", "прокальцитонин",
    "ретикулоцит", "липаз",
    "магний", "фосфор",
    "витамин d", "витамин b12", "фолиевая кислот",
    "простатический",
    "удельный вес мочи", "белок в моче", "кетоновые тел",
]
...
_NOISE_PREFIXES = (
    ...
    "значение",
    "дата исследования",
    # Шкальные пояснения (Гемотест и др.)
    "нормальный уровень",
    "умеренно-повышенный",
    "умеренно повышенный",
    "повышенный ",       # с пробелом, чтобы не зацепить "Повышенный холестерин..."
    "высокий уровень",
    "высокий риск",
    "умеренный риск",
    "риск отсутствует",
    ...
    # P6: единицы с пояснениями / HbA1c шкала
    "ммоль/л -",
    "ммоль/л —",
    "ммоль/л–",
    "в соответствии с",
    ...
    "6.0-6.4%",
    "6.5% и более",
    # P6: мусорные фрагменты, пробившиеся как имена кандидатов
    "мз рф",
    "(сыворотка",
    "(венозная кровь",
    "(цельная кровь",
...
def is_noise(line: str) -> bool:
    ...
    for prefix in _NOISE_PREFIXES:
        if low.startswith(prefix):
            return True
    # Шкальные аннотации: строка начинается с ">" или "<" и содержит текст-описание уровня
    if re.match(r'^[><≤≥]\s*\d', s) and re.search(r'(риск|уровень|ммоль|норм)', low):
        return True
